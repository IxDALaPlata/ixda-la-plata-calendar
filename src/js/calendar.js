...
function renderCalendars(calendars) {
	return html `<div class="fade-in ${calendars.length ? 'db' : 'dn'}">
				...
				${calendar.when.month + ' ' + calendar.when.year}</h2>
				${renderWeekdays()}
...
function renderError(show) {
	return html `<div class="fade-in f4 f3-ns pv5 silver tc ${show ? 'db' : 'dn'}">
				...
				<button class="b b--black-10 ba black-alternative br2 bw1 f6 grow link mt4 ph3 pointer pv2 ttu hover-light-blue-ixda"
						onclick="${lookForData}">
								Intentar nuevamente
				</button>
		</div>`
	...
	return html `<div class="bg-black-70 fade-in fixed items-center justify-center left-0 pointer top-0 vh-100 w-100 z-2 ${
				...
				<div id="modal-wrapper" class="center cursor-default fade-in-down mw7 w-100">
						<div class="bg-white br2 ma3">
								<div class="b--black-10 bb bg-white br--top br2 bw1 flex items-center justify-between ph3 pv2">
...
function renderModalEvent(event) {
	return html `<div class="flex mh3 mv3 pv3">
				...
				<div class="w-70 w-80-ns">
						...
						${event.description ? html`<p class="black-30 mb0 mt2">${event.description}</p>` : ''}
						${event.place ? html`<p class="mb0 mt2">${event.place}</p>` : ''}
						${event.place ? html`<a class="mb0 mt2 link"
																		target="_blank"
																		href="https://www.google.com/maps/search/?api=1&query=${event.place.replace(/ /g,"+")}">
